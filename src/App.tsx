...
export default function App() {
  ...
  const handleFiles = (files: FileList) => {
    ...
    const processCsvText = (csvText: string, label: string, sheetName?: string) => {
      ...
      if (isLeg2Sheet) {
        // The sheet itself is dedicated to a single specific leg (e.g. a
        // separate "Leg 2" tab) - parse just that one leg.
      ...
      // Otherwise, this sheet may contain multiple leg columns side by side
      // (Leg 1, Leg 2, Leg 3, ... however many). Detect the real count from
      // the number of "NPBR Qual Matrix Part#" columns rather than guessing
      // from which leg-number words happen to appear in the text - a sheet
      // whose legs are numbered e.g. "Leg 4".."Leg 7" contains none of the
      // literal strings "leg 2" / "leg 3", so a text search would miss all
      // of them.
      const legCount = detectLegCount(csvText);
      for (let legIndex = 1; legIndex <= legCount; legIndex++) {
        const parsedLeg = parseLVBuildMatrix(csvText, label, legIndex);
        if (parsedLeg && isValidParsedBuild(parsedLeg)) {
          newBuilds.push(parsedLeg);
        }
      }
    };
